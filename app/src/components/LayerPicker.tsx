...
export function LayerPicker({ themes, activeLayers, onToggle }: LayerPickerProps) {
  ...
  return (
    <>
      ...
      >
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" className="text-sepia">
          <path d="M8 1L1 5l7 4 7-4-7-4z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" />
          <path d="M1 8l7 4 7-4" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" />
          <path d="M1 11l7 4 7-4" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" />
        </svg>
        <span className="text-ink font-medium">Layers</span>
        <span className="text-faded">{activeLayers.size}/{themes.length}</span>
      </button>
      ...
      {open && (
        <div className="absolute top-14 left-3 z-20 bg-paper border border-sepia-light rounded-lg shadow-lg w-72 overflow-hidden">
          <div className="px-3 py-2 border-b border-sepia-light flex items-center justify-between">
            <span className="text-xs text-faded uppercase tracking-wider">Themes</span>
            <button
              onClick={() => {
              ...
              return (
                ...
                >
                  <span
                    ...
                    style={{
                      backgroundColor: active ? themeColor(theme.slug) : "transparent",
                      borderColor: themeColor(theme.slug),
                      opacity: active ? 1 : 0.4,
                    }}
                  />
                  <span className={`text-sm flex-1 ${active ? "text-ink" : "text-faded"}`}>
                    {theme.title}
                  </span>
                  <span className="text-xs text-faded tabular-nums">{theme.poi_count}</span>
                </button>
              );
            })}
          </div>
